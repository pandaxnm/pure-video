...
class Utils
{
    ...
    */
    ...
    private $name = 'manfoll';
    private $domain = '.manfoll.video';
    private $expire = 604800;//7天
    private $path = '/';
    private $secure = false;
    private $httpOnly = true;
    private $key = 'sv7nNmzFgYU5zEWAVIzRL4BnVoUqrklB';
    ...
    public function readCookie() {
        ...
        if (isset($cookies[$name]) && $cookies[$name] != '') {
            return $this->validateData($cookies[$name], $this->key);
        } else {
            return false;
        }
     ...
     *
     * @param  array  $data
     * @param  boolen $remeber
     * @return
     */
    public function writeCookie($data, $remember = true) {
        if (empty($data))
            return false;

        $cookieValue = $this->hashData($data, $this->key);
        if (!$cookieValue)
            return false;

        $expire = $remember ? time() + $this->expire : 0;
        setcookie($this->name, $cookieValue, $expire, $this->path, $this->domain, $this->secure, $this->httpOnly);
        return true;
    }
    ...
    public function removeCookie() {
        setcookie($this->name, '', time() - 100, $this->path, $this->domain, $this->secure, $this->httpOnly);
        return true;
    }
    ...
    private function hashData($data, $key, $rawHash = false) {
        $data = serialize($data);
        $hash = hash_hmac('sha256', $data, $key, $rawHash);
        if (!$hash) {
            return false;
        }
        return $hash . $data;
     ...
     * @return
     */
    private function validateData($data, $key, $rawHash = false) {
        $test = @hash_hmac('sha256', '', '', $rawHash);
        if (!$test) {
            return false;
        }
        ...
        if ($this->byteLength($data) >= $hashLength) {
            ...
            $calculatedHash = hash_hmac('sha256', $pureData, $key, $rawHash);

            if ($this->compareString($hash, $calculatedHash)) {
                return unserialize($pureData);
            }
        }
        return false;
    }

    private function byteLength($string) {
